...
import Shop_FilterPanel from "../../components/Shop/Shop_FilterPanel";
import SearchItems from "../../components/Shop/SearchItems";
import { GridToggler } from "../../components/Shop/GridToggler";
...
export default function Shop() {
  ...
  const currentPriceRange = searchParams
    .get("priceRange")
    ?.split("-")
    .map(Number) || [0, 100000];

  const params: TQueryParams[] | undefined =
    queryArray.length > 0 ? queryArray : [];
  const finalParams = [
    ...params,
    {
      name: "fields",
      value:
        "name,price,category,images,quantity,brand,frameMaterial,specifications",
    },
  ];
  ...
  const handleFilterCategory = (
    category: (typeof productCategories)[number] | "all"
  ) => {
    if (category === "all") {
  ...
  const handleFilterFrame = (
    frameMaterial: (typeof productFrameMaterial)[number] | "all"
  ) => {
    ...
    <section className="py-8 flex gap-6">
      ...
      <div className="relative w-full">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {categoryFilter && (
              <Badge variant="secondary" className="flex items-center gap-1">
                {categoryFilter}
                <button onClick={() => handleFilterCategory("all")}>
            ...
            {frameFilter && (
              <Badge variant="secondary" className="flex items-center gap-1">
                {frameFilter}
                <button onClick={() => handleFilterFrame("all")}>
                  <X className="h-3 w-3 ml-1 cursor-pointer" />
                </button>
              </Badge>
            )}
            ...
            {searchParams.get("priceRange") && (
              <Badge variant="secondary" className="flex items-center gap-1">
                Price: ৳{currentPriceRange[0].toLocaleString()} - ৳
                {currentPriceRange[1].toLocaleString()}
        ...
        <div className="flex-1">
          {isLoadingData ? (
            <div
              className={`grid grid-cols-${gridNumberSmallDevice} lg:grid-cols-${gridNumber} gap-6 `}
            >
              {Array(6)
                .fill(0)
                .map((_, i) => (
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="lg:hidden">
                  <GridToggler
                    gridNumber={gridNumberSmallDevice}
                    setGridNumber={setGridNumberSmallDevice}
                    totalGrid={2}
                    increaseBy={1}
                  />
                </div>
              ...
              {meta && (
                <DefaultPagination meta={meta} showItemPerPage={false} />
              )}
            </>
          )}
        </div>
