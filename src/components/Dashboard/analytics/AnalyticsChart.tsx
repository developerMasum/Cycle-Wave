...
export default function AnalyticsChart() {
  const { data } = useGetLast12MonthsAnalyticsDataQuery(undefined);
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();

  const chartTypeParam = searchParams.get("chartType");
  const chartType: ChartKey = ["orders", "revenue", "users"].includes(
  ...
  const chartView: ChartView = ["line", "bar"].includes(
      ...
      <CardContent>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-4">
            ...
            <Select
              value={chartType}
              onValueChange={(val) => handleSelectChange("chartType", val)}
            >
            ...
            <Select
              value={chartView}
              onValueChange={(val) => handleSelectChange("chartView", val)}
            >
